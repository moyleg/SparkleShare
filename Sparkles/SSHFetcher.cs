...
namespace Sparkles {
    ...
    public abstract class SSHFetcher : BaseFetcher {
        ...
        string FetchHostKey ()
        {
            ...
            // Use the actual port from the URL, defaulting to 22 for SSH if not specified
            int port = RemoteUrl.Port > 0 ? RemoteUrl.Port : 22;
            var ssh_keyscan = new Command (SSHKeyScan, string.Format ("-t rsa -p {0} {1}", port, RemoteUrl.Host));
